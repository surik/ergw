...
-export([get_request_q/1, get_response_q/1, get_seq_no/2, 
	 get_redirector_nodes/1, set_redirector_nodes/2,
...
-record(state, {
      ...
      redirector  = nil :: gtp_redirector:redirector(),
...
get_redirector_nodes(Pid) ->
    gen_server:call(Pid, get_redirector_nodes).
...
handle_call(get_redirector_nodes, _From, 
    ...
    Result = #{name => Name, 
               redirector_nodes => [#{ip => IP,
                                      version => Version}
                                    || {Version, IP} 
                                       <- gtp_redirector:get_nodes(Redirector)]},
    {reply, Result, State};

handle_call({set_redirector_nodes, Nodes}, _From, #state{redirector = Redirector} = State) ->
    NewRedirector = gtp_redirector:set_nodes(Redirector, Nodes),
    {reply, ok, State#state{redirector = NewRedirector}};
...
handle_info({timeout, _TRef, redirector_keep_alive}, 
            #state{gtp_port = GtpPort,
                   redirector = Redirector} = State) when Redirector /= nil ->
...
handle_info({timeout, _TRef, redirector_requests}, 
            #state{redirector = Redirector} = State) when Redirector /= nil ->
...
handle_message_1(_ArrivalTS, IP, Port, 
                 ...
                 #state{gtp_port = GtpPort,
                        redirector = Redirector} = State) 
  when Redirector /= nil, Type /= echo_response, Type /= echo_request ->
